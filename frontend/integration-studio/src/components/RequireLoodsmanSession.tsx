...
interface RequireLoodsmanSessionProps {
  children: ReactNode
}

/**
 * Клиентская охрана защищённой зоны loodsman.
 * Пока сессия проверяется — полноэкранный спиннер.
 * Бэкенд недоступен (сетевая ошибка) — блок с кнопкой «Повторить», без редиректа.
 * Сессии нет — редирект на /loodsman/auth (на время замены — null).
 * Сессия есть — рендерим содержимое.
 */
const RequireLoodsmanSession = ({ children }: RequireLoodsmanSessionProps) => {
  const router = useRouter()
  const { data, isLoading, isFetching, isError, refetch } = useLoodsmanSession()
...
export default RequireLoodsmanSession
