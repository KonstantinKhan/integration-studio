...
const PolynomAuthPage = () => {
  ...
  const selectedStorageId = useAuthStore((s) => s.selectedStorageId)
  ...
  const selectedStorage = storages?.find(
    (storage) => storage.storageId === selectedStorageId,
  )
  ...
  const handleSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (!selectedStorageId) {
      setAuthError('Выберите хранилище')
      setStep(1)
      return
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <span className="text-sm font-medium text-stone-700 truncate">
                {selectedStorage ? selectedStorage.displayName : selectedStorageId}
              </span>
              <button
                type="button"
                onClick={goBackToStorage}
...
export default PolynomAuthPage
