...
Поток типичного запроса: route (ktor-server-app) → `PolynomApplicationService` (logics) → `PolynomClient`/sub-API (polynom-client) → Polynom; ответ маппится в domain (mapping) и сериализуется в BFF-ответ.
...
Токены Polynom хранятся внутри `UserCredentials` в сессии; refresh — см. [Конфигурация](configuration.md).
